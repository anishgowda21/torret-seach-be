 ...
 */
export const get1337xDocs = createDocumentationHandler('../docs/1337x.md', '1337x API Documentation');
...
export const search1337x = async (req: Request, res: Response) => {
  ...
  try {
    const cacheKey = `search:${trimmedQuery.toLowerCase()}:${pageNum}:${category || 'all'}:${sort || 'seeders'}:${orderStr}`;
